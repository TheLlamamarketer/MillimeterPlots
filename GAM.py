...
def d_E(N):
    return np.sqrt((d_intercept / slope)**2 + ((N - intercept) / slope**2 *d_slope)**2)

print(f"$E = {E(1377):.3f} \\pm {d_E(1377):.3f}$")

print(f"Slope: {slope:.1f} \\pm {d_slope:.1f}, Intercept: {intercept:.1f} \\pm {d_intercept:.1f}")
...
def print_table(data):
    ...
    print("\\begin{table}[h!]")
    print("    \\centering")
    print("    \\caption{{Messdaten zur Analyse verschiedener Isotope ($a$ sind die gemessenen Werte der Intensität, $b$ die Abweichungen und $E$ die Energie in MeV)}}")
    print("    \\sisetup{table-format=4.1}")
    print("    \\begin{tabular}{| *{1}{S[table-format=4.1]} | *{3}{S[table-format=4.1]} |}")
    print("    \\toprule")
...
#plot_data(ymin=0, ymax=3640, ystep=130, xmin=0, xmax=1.4, xstep=0.14, x_label='E/MeV', y_label='N/#', datasets=[{'xdata':data['E'], 'ydata': data['a'] , 'y_error': data['b']}], filename='Plots/GAM_1.pdf')
...
def print_table(data_pb, data_fe):
    ...
    print("    \\begin{tabular}{| *{1}{S[table-format=2.1]} *{1}{S[table-format=5.0]} *{1}{S[table-format=2.1]} | *{1}{S[table-format=2.1]} *{1}{S[table-format=5.0]} *{1}{S[table-format=2.1]} |}")
    print("    \\toprule")
...
slope_pb = np.sum((np.array(data_pb['A']) - np.mean(data_pb['A'])) * (np.array(data_pb['d']) - np.mean(data_pb['d']))) / np.sum((np.array(data_pb['d']) - np.mean(data_pb['d']))**2)
...
d_slope_pb = np.sqrt(np.sum((np.array(data_pb['A']) - slope_pb * np.array(data_pb['d']) - intercept_pb)**2) / (len(data_pb['A']) - 2) / np.sum((np.array(data_pb['d']) - np.mean(data_pb['d']))**2))
d_slope_fe = np.sqrt(np.sum((np.array(data_fe['A']) - slope_fe * np.array(data_fe['d']) - intercept_fe)**2) / (len(data_fe['A']) - 2) / np.sum((np.array(data_fe['d']) - np.mean(data_fe['d']))**2))
...
#plot_data(ymin=7.7, ymax=10.5, ystep=0.1, xmax=35, xmin=3, xstep=2, grey_zones=grey_zones, x_label='d/mm', y_label='I/ #/s', filename='Plots/GAM_4.pdf', datasets=[{'xdata': data_pb['d'], 'ydata': data_pb['A'], 'x_error': [0.1]*len(data_pb['d']), 'color': '#003366', 'label':'Blei'}, {'xdata': data_fe['d'], 'ydata': data_fe['A'], 'x_error': [0.1]*len(data_fe['d']), 'color': '#006400', 'label':'Eisen'}] )
...
from plotting import generate_contrasting_color
