...
dT_p = {
    f"{g}_T_{p}": np.sqrt(
        sum(
            (T[f"{g}_T_{p}"][i] - np.mean(T[f"{g}_T_{p}"][:10])) ** 2 for i in range(10)
        )
        / (90)
    )
    for g in gas
...
dT = {
    f"{g}_T_{p}": np.sqrt(
        sum(
            (T[f"{g}_T_{p}"][i] - np.mean(T[f"{g}_T_{p}"][:10])) ** 2 / p
            for i in range(10)
        )
        / (90)
    )
...
T_avg = {
    f"{g}_{p}": sum(T[f"{g}_T_{p}"][:10]) / (len(T[f"{g}_T_{p}"][:10]) * p)
    for g in gas
    for p in periods
}
...
k = {
    f"{g}_{p}": (64 * m[g] * V[g]) / (d**4 * P * (T_avg[f"{g}_{p}"]) ** 2)
    for g in gas
    for p in periods
}
...
dk_avg = {
    g: np.sqrt(sum([dk[f"{g}_{p}"] ** 2 for p in periods])) / len(periods) for g in gas
}
...
def print_mini_table(m, V, gas):
    ...
    print("    \\centering")
    print(
        "    \\caption{Gerätedaten über die drei verwendeten Gasoszillatoren (für jede Gasflasche einen Aufbau mit Arbeitsvolumen V, Kolbenmasse m)}"
    )
    print("    \\sisetup{table-format=2.2}")
    print(
        "    \\begin{tabular}{| *{2}{S[table-format=2.2]} | *{2}{S[table-format=2.2]} | *{2}{S[table-format=2.2]} |}"
    )
    print("    \\toprule")
    print(
        "    \\multicolumn{2}{|c|}{Ar} & \\multicolumn{2}{c|}{CO2} & \\multicolumn{2}{c|}{N2} \\\\"
    )
    print("    \\midrule")
    print("    \\midrule")
...
def print_result_table(k, dk, gas, periods):
    ...
    print("    \\centering")
    print(
        "    \\caption{Berechnete Werte für die Isotropenexponenten $\kappa_{Gas}$ und ihre Fehler $\Delta \kappa_{Gas}$ für die Gase Argon (Ar), Kohlenstoffdioxid (CO2) und Stickstoff (N2) nach Auswertung der Gasoszillator Messwerte für die Dauer T von 50 bzw. 100 Perioden}"
    )
    print("    \\sisetup{table-format=2.3}")
    print(
        "    \\begin{tabular}{|l | *{2}{S[table-format=2.3]} | *{2}{S[table-format=2.3]} | *{2}{S[table-format=2.3]} |}"
    )
    print("    \\toprule")
    ...
    for p in periods:
        row_data = " & ".join(
            [f"{k[f'{g}_{p}']:.3f} & {dk[f'{g}_{p}']:.3f}" for g in gas]
        )
...
def print_table(T):
    ...
    print("    \\centering")
    print(
        "    \\caption{Messdaten Gasoszillator für Argon (Ar) Kohlenstoffdioxid (CO2) und Stickstoff (N2)($T_j$ ist die Dauer von j Perioden) und einem Außenluftdruck von $p_A=(1\pm0,001)$bar}"
    )
    print("    \\sisetup{table-format=2.2}")
    print(
        "    \\begin{tabular}{| *{2}{S[table-format=2.2]} | *{2}{S[table-format=2.2]} | *{2}{S[table-format=2.2]} |}"
    ...
    print(f"    {first} \\\\")
    print(
        f"    {T_avg['Ar_50']*50:.3f} & {T_avg['Ar_100']*100:.3f} & {T_avg['CO2_50']*50:.3f} & {T_avg['CO2_100']*100:.3f} & {T_avg['N2_50']*50:.2f} & {T_avg['N2_100']*100:.3f} \\\\"
    )
    print(f"    \\midrule")
    middle = " & ".join(
        [f"{{$\\Delta\\bar{{T}}_{{{p}}}$ (s)}}" for _ in gas for p in periods]
    )
    print(f"    {middle} \\\\")
    print(
        f"    {dT_p['Ar_T_50']:.3f} & {dT_p['Ar_T_100']:.3f} & {dT_p['CO2_T_50']:.3f} & {dT_p['CO2_T_100']:.3f} & {dT_p['N2_T_50']:.2f} & {dT_p['N2_T_100']:.3f} \\\\"
    )
    print(f"    \\midrule")
    ...
    print(f"    {bottom} \\\\")
    print(
        f"    {T_avg['Ar_50']:.4f} & {T_avg['Ar_100']:.4f} & {T_avg['CO2_50']:.4f} & {T_avg['CO2_100']:.4f} & {T_avg['N2_50']:.3f} & {T_avg['N2_100']:.4f} \\\\"
    )
    print(f"    \\midrule")
    ...
    print(f"    {last} \\\\")
    print(
        f"    {dT['Ar_T_50']:.4f} & {dT['Ar_T_100']:.4f} & {dT['CO2_T_50']:.4f} & {dT['CO2_T_100']:.4f} & {dT['N2_T_50']:.3f} & {dT['N2_T_100']:.4f} \\\\"
    )
    print("    \\bottomrule")
    print("    \\end{tabular}")
    print("    \\label{tab:A2}")
