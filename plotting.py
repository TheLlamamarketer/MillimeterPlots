...
def apply_si_format(
    ...
    *,
    ...
    ax.set_xlabel(_ensure_unit(ax.get_xlabel(), xunit))
    ax.set_ylabel(_ensure_unit(ax.get_ylabel(), yunit))
    ...
    *,
    ...
    with mpl.rc_context(rc):
        ...
        if xticks is not None and yticks is not None and xlim is not None and ylim is not None:
            ...
            gray = mcolors.to_rgb("#2a3140")
            ax.grid(True, which="major", color = _ensure_contrast(gray, gray))
            ax.grid(True, which="minor", linestyle=":", linewidth=0.6, alpha=0.8, color = _ensure_contrast(gray, gray))
        ...
        for s in specs:
            ...
            if s.xerr is not None or s.yerr is not None:
                eb = ax.errorbar(
                    ...
                    label=s.label)
                if outline:
                    for art in eb.lines + eb.caplines:
    ...
    with mpl.rc_context({**(_LIGHT_RC if _auto_style_from_bg(bg_hex) == "light" else _DARK_RC)}):
        ...
        for col, seed in enumerate(seeds):
            ...
            for row in range(num_groups):
                ax.add_patch(Rectangle((col, row), 0.8, 0.8, color=cols[row]))
